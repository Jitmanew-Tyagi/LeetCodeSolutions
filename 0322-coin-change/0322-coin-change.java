class Solution {
    ...
    public static int rec(int n, int[] coins, int i, int amount, int noc, int[][] dp) {
        if(amount == 0) return dp[i][amount] = 0;
        if(i == n || amount < 0) return Integer.MAX_VALUE - 1;
        if(dp[i][amount] != -1) return dp[i][amount];
        int a1 = rec(n, coins, i + 1, amount, noc, dp);
        int a2 = 1 + rec(n, coins, i, amount - coins[i], noc + 1, dp);
